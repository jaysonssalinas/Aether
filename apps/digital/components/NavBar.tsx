...
export function NavBar() {
  ...
  return (
    <nav
      ...
      style={{ background: 'rgba(10,10,10,0.85)', backdropFilter: 'blur(20px)', borderBottom: '1px solid #1e1e1e' }}
    >
      ...
      {menuOpen && (
        <div className="md:hidden px-6 pb-6 flex flex-col gap-4" style={{ borderTop: '1px solid #1e1e1e' }}>
          {links.map((l) => (
            <Link key={l.href} href={l.href} className="text-sm py-2" style={{ color: '#999999' }} onClick={() => setMenuOpen(false)}>
              {l.label}
            </Link>
          ))}
        </div>
